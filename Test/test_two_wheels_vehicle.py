...
vehicle = TwoWheelsVehicle(pose, spec)
...
def test_attributes():
    ...
    assert hasattr(vehicle, "front_tire") == True
    assert hasattr(vehicle, "rear_tire") == True
...
def test_initialize():
    ...
    assert vehicle.front_tire != None
    assert vehicle.rear_tire != None
...
def test_set_agent_motion():
    ...
    vehicle = TwoWheelsVehicle(pose, spec, agent=agent, motion=motion)
