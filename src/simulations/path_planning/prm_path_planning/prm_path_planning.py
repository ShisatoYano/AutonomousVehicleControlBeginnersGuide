...
sys.path.append(abs_dir_path + relative_path + "plan/prm")
...
from prm_path_planner import PrmPathPlanner
...
def main():
    ...
    sim_dir = (abs_dir_path + relative_simulations
               + "path_planning/prm_path_planning/")
    map_path = sim_dir + "map.json"
    path_filename = sim_dir + "path.json"
    search_gif_path = sim_dir + "prm_search.gif"
    navigate_gif_path = sim_dir + "prm_navigate.gif"
    ...
    if not planner.path:
        print("PRM: no path found – aborting.")
        return
