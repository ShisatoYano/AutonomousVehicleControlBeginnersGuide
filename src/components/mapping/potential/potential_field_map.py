...
class PotentialFieldMap:
    ...
    def _initialize_caches(self):
        ...
        for vector_idx in range(num_grids):
            center_x, center_y = self.map.calculate_grid_center_xy_pos_from_vector_index(
                vector_idx)
            self._attractive_cache[vector_idx] = self._compute_attractive_potential(
                center_x, center_y)
    ...
    def _discretize_position(self, x_m, y_m):
        ...
        vector_idx = self.map.calculate_vector_index_from_xy_index(
            x_idx, y_idx)
    ...
    def _get_affected_grid_range(self, obs_x, obs_y):
        ...
        if max_x_idx is None:
            max_x_idx = self.map.width_grids_num - 1
        if min_y_idx is None:
            min_y_idx = 0
        if max_y_idx is None:
            max_y_idx = self.map.height_grids_num - 1
    ...
    def _add_obstacle_repulsion(self, obs_x, obs_y):
        ...
        """
        min_x_idx, max_x_idx, min_y_idx, max_y_idx = self._get_affected_grid_range(
            obs_x, obs_y)
        eps = 1e-9
        ...
        for x_idx in range(min_x_idx, max_x_idx + 1):
            for y_idx in range(min_y_idx, max_y_idx + 1):
                vector_idx = self.map.calculate_vector_index_from_xy_index(
                    x_idx, y_idx)
                center_x, center_y = self.map.calculate_grid_center_xy_pos_from_xy_index(
                    x_idx, y_idx)
    ...
    def calculate_repulsive_potential(self, x_m, y_m, obstacle_positions):
        ...
        for obs_x_m, obs_y_m in obstacle_positions:
            ...
            if d <= self.rho:
                u_rep_total += 0.5 * self.eta * \
                    (1.0/(d + eps) - 1.0/self.rho)**2
        return u_rep_total
    ...
    def draw_map(self, axes, elems, colormap='jet'):
        ...
        x_range = np.arange(self.map.width_grids_num + 1) * \
            self.map.resolution_m + self.map.left_bottom_x_m
        y_range = np.arange(self.map.height_grids_num + 1) * \
            self.map.resolution_m + self.map.left_bottom_y_m
        ...
        pcm = axes.pcolormesh(X, Y, Z, cmap=colormap,
                              alpha=0.5, shading='flat')
        elems.append(pcm)

    def export_to_json(self, filepath):
        """
        Export potential field grid with metadata to JSON file.

        This enables decoupled path planning - the map generator and path
        planner can be separate modules that communicate via JSON.

        JSON Schema:
        {
            "metadata": {
                "resolution": float,  # Grid cell size in meters
                "width": int,         # Number of columns (x-axis)
                "height": int,        # Number of rows (y-axis)
                "origin": [x, y]      # World coords of bottom-left corner
            },
            "goal_pos": [x, y],       # Goal position in world coords
            "potential_grid": [[...]] # 2D array where grid[row][col] = grid[y][x]
        }
        ...
        """
        # Convert 1D grid data to 2D array (height x width)
        # The grid uses row-major ordering: vector_idx = y_idx * width + x_idx
        # So we iterate y (rows) in outer loop, x (cols) in inner loop
        potential_2d = []
        for y_idx in range(self.map.height_grids_num):
            row = []
            for x_idx in range(self.map.width_grids_num):
                vector_idx = self.map.calculate_vector_index_from_xy_index(
                    x_idx, y_idx)
                value = self.map.get_grid_data(vector_idx).get_data()
                row.append(value)
            potential_2d.append(row)

        # Package all data needed to reconstruct the coordinate system
        data = {
            "metadata": {
                "resolution": self.map.resolution_m,
                "width": self.map.width_grids_num,
                "height": self.map.height_grids_num,
                "origin": [self.map.left_bottom_x_m, self.map.left_bottom_y_m]
            },
            "goal_pos": [self.goal_x_m, self.goal_y_m],
            "potential_grid": potential_2d
        }
