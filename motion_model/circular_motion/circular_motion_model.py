...
def main():
    ...
    # 円運動モデルのオブジェクト
    cmm = CircularMotionModel(interval_sec=INTERVAL_SEC)

    # 四輪モデル車両の描画オブジェクト
    fwv = FourWheelsVehicle(ax)

    # Gif作成クラスのインスタンス生成
    save_name_path = os.path.dirname(os.path.abspath(__file__)) + "/../../gif/circular_motion_model.gif"
    ...
    while TIME_LIMIT_SEC >= elapsed_time_sec:
        ...
        # 運動モデルによる計算
        x_m, y_m, yaw_deg, steer_deg = cmm.calculate_state(x_m, y_m, yaw_deg, speed_input, yaw_rate_input)
